...
def data_load(filename, axisname, label):
    ...
    while end <= fl.shape[0]/10:
        x = fl[start:end]
        imgs = CWT(signal_size + 1, x)  # signal_size * signal_size (100 * 100)
        data.append(imgs)
        lab.append(label)
        start += signal_size
        end += signal_size
    ...
    def data_preprare(self, test=False):
        if len(os.path.basename(self.data_dir).split('.')) == 2:
            with open(self.data_dir, 'rb') as fo:
                list_data = pickle.load(fo, encoding='bytes')  # 加载
        else:
            list_data = get_files(self.data_dir, test)  # CWT 变换
            with open(os.path.join(self.data_dir, "CWRUCWT.pkl"), 'wb') as fo:
                pickle.dump(list_data, fo)  # 保存
        if test:
